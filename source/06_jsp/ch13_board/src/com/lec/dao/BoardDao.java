...
//	-- 2. 글목록
	public ArrayList<BoardDto> listBoard() {
		...
		try {
			...
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			while(rs.next()) { // 0줄이면 한번도 실행하지 않는다(데이터가 없을때까지)
...
//	-- 4. 글번호로 글상세보기 내용(DTO) 가져오기 : SELECT * FROM BOARD WHERE NUM=2;
	public BoardDto getBoardOneLine(String numStr) {
		...
		try {
			...
			rs = pstmt.executeQuery();
			if(rs.next()) { // 1줄만 나오므로 while문에서 -> if문으로 수정
				int num = rs.getInt("num");
				String writer = rs.getString("writer");
				String subject = rs.getString("subject");
				String content = rs.getString("content");
...
//	-- 5. 조회수 올리기
	public void readCountUp(String num) { // return값을 받는건 static메소드에서만
		Connection conn = null;
		PreparedStatement pstmt = null;
		...
		try {
			...
			pstmt.executeUpdate();
			// int result = pstmt.executeUpdate(); // 윗줄이나 이 줄 둘중 하나 둘다 키면 조회수 2증가
	...
	public void readCountUp(int num) { // return값을 받는건 static메소드에서만
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = "UPDATE BOARD SET READCOUNT = READCOUNT + 1 WHERE NUM = ?";
		try {
