...
public class TV implements IVolume{
	...
	public void volumDown() {
		if(volumeLevel > MIN_VOLUME) { 
			volumeLevel --;
			System.out.println("TV 볼륨을 1만큼 내려 현재 볼륨" + volumeLevel);
		}else {
	...
	public void volumDown(int level) {
		if(volumeLevel-level > MIN_VOLUME) { // 현재 볼륨이 5, level 2 
			volumeLevel -= level;
			System.out.println("TV 볼륨을" +level+ "만큼 내려 현재 볼륨" + volumeLevel);
		}else { // 현재 볼륨 5, level 7
			int tempLevel = volumeLevel - MIN_VOLUME;
			volumeLevel = MIN_VOLUME;
			System.out.println("TV 볼륨을" + tempLevel + "만큼 내려 최소입니다");
		}
	}
}
