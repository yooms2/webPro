...
-- SAL의 평균, 합, 최소값, 최대값, 분산, 표준편차, 개수(평균, 분산, 표준편차 소수점 한자리에서 반올림)
...
SELECT MIN(HIREDATE)||':'||TRUNC(SYSDATE-MIN(HIREDATE))||'일째',
    MAX(HIREDATE)||':'||TRUNC(SYSDATE-MAX(HIREDATE))||'일째' FROM EMP;
-- 탄탄ex3.  (결과) 80년12월17일 최초입사:14,620일째 83년01월12일 최근입사 :13,864일째
SELECT MIN(TO_CHAR(HIREDATE, 'RR"년"MM"월"DD"일 최초입사:"')) ||
        TRIM(TO_CHAR(TRUNC(SYSDATE-MIN(HIREDATE)), '99,999')) || '일째' FIRST,
    MAX(TO_CHAR(HIREDATE, 'RR"년"MM"월"DD"일 최초입사:"')) ||
...
SELECT DEPTNO, MAX(SAL), AVG(SAL) FROM EMP GROUP BY DEPTNO ORDER BY DEPTNO;
    -- GROUP BY절에서는 컬럼의 별칭을 사용할 수 없다(ORDER BY절에서만 컬럼별칭 사용가능)
SELECT DEPTNO 부서, MAX(SAL), AVG(SAL) FROM EMP GROUP BY DEPTNO ORDER BY 부서;
--             3                        1         2                4

-- 부서이름별 최대급여, 평균급여 (평균급여순 정렬)
...
SELECT DEPTNO, AVG(SAL) AVG_SAL
    FROM EMP
    GROUP BY DEPTNO
    HAVING AVG(SAL) > 2000 -- HAVING절에서 필드의 별칭 사용 불가
    ORDER BY DEPTNO; -- 필드의 별칭은 ORDER BY절에서만 사용 가능
...
-- ★ 4. 결과 집합 낸 집계값 생성
SELECT DEPTNO, SUM(SAL) FROM EMP GROUP BY ROLLUP(DEPTNO); -- ROLLUP은 자동으로 오름차순
...
SELECT MAX(SAL)-MIN(SAL) "최대,최소 급여 차이" FROM EMP;
